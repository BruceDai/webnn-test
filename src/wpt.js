...
class WptRunner extends WebNNRunner {
  writeFinalResultsCsv(results) {
    ...
    const getMessage = (result) => {
      ...
      if (
        Array.isArray(result.failedSubtests) &&
        result.failedSubtests.length > 0
      ) {
        return result.failedSubtests
          .map(
            (s) => `${s.name || "subtest"}: ${s.message || s.status || "FAIL"}`,
          )
          .join(" | ");
      }
      return "";
    ...
    const lines = [];
    lines.push(
      ["Backend", "Test Suite", "Test Case", "Status", "Message"]
        .map(csvEscape)
        .join(","),
    );
    ...
    const runDir =
      process.env.PROJECT_RUN_DIR || path.join(__dirname, "..", "results");
    const timestamp = process.env.PROJECT_TIMESTAMP;
    const configFileName =
      (process.env.CURRENT_CONFIG_FILE_NAME || "").trim() || "config";
  ...
  async runWptTests(context, browser, onFirstCaseComplete) {
    ...
    if (rangeFilter) {
      ...
      console.log(
        `Debug: Parsed range filter. Selected indices count: ${selectedIndices.size}`,
      );
    }
    ...
    console.log(
      `Running WPT tests. Case: ${wptCase || "ALL"}, Range: ${rangeFilter || "ALL"}, Jobs: ${jobs}`,
    );
    ...
    console.log(
      `[Success] Test discovery complete. Found ${testFiles.length} files.`,
    );
    ...
    console.log(
      `[Info] Starting test run with ${testFiles.length} tests. Parallel jobs: ${jobs}`,
    );
    ...
    if (enableGpuCrashLogHandler) {
      try {
        ...
        console.log(
          `[Warning] Failed to initialize GPU crash log baseline: ${e.message}`,
        );
      }
    } else {
          ...
          await test.step(`Test: ${testFile}`, async () => {
            ...
            try {
              ...
              try {
                  ...
                  new Promise((_, reject) =>
                    setTimeout(
                      () =>
                        reject(
                          new Error(
                            "restartBrowserAndContext timeout 45000ms exceeded",
                          ),
                        ),
                      45000,
                    ),
                ...
                console.log(
                  `[Fail] Browser relaunch hung: ${restartErr.message}. Force-killing stray processes...`,
                );
                try {
                  this.forceKillBrowserProcessTree("relaunch hang");
                } catch (e) {
                  console.log(
                    `[Debug] relaunch-hang forceKill failed: ${e && e.message ? e.message : e}`,
                  );
                }
                throw restartErr;
              } finally {
              ...
              page = await Promise.race([
                ...
                new Promise((_, reject) =>
                  setTimeout(
                    () => reject(new Error("newPage timeout 15000ms exceeded")),
                    15000,
                  ),
                ),
              ]);
              ...
              const res = await Promise.race([
                ...
                new Promise((_, reject) => {
                  const timeoutError = new Error(
                    "Test timeout 120000ms exceeded",
                  );
              ...
              if (enableGpuCrashLogHandler) {
                ...
                try {
                    ...
                    new Promise((_, reject) =>
                      setTimeout(
                        () =>
                          reject(
                            new Error(
                              "getGpuCrashLogInfo timeout 15000ms exceeded",
                            ),
                          ),
                        15000,
                      ),
                  ...
                  console.log(
                    `[Warning] getGpuCrashLogInfo hung (${gpuErr.message}). Treating as GPU crash and force-killing browser.`,
                  );
                  try {
                    this.forceKillBrowserProcessTree("getGpuCrashLogInfo hang");
                  } catch (e) {
                    console.log(
                      `[Debug] getGpuCrashLogInfo-hang forceKill failed: ${e && e.message ? e.message : e}`,
                    );
                  }
                  res.crashed = true;
                  res.result = "CRASH";
                ...
                if (gpuLog.crashCount > lastGpuCrashCount) {
                  ...
                  const crashDetails =
                    Array.isArray(gpuLog.crashDetails) &&
                    gpuLog.crashDetails.length > 0
                  ...
                  console.log(
                    `[Fail] ${crashLabel} ${testFile}: GpuProcessHost crash detected in chrome://gpu logs.`,
                  );
                }
                lastGpuCrashCount = Math.max(
                  lastGpuCrashCount,
                  gpuLog.crashCount,
                );
              }
              ...
              const isTimeoutError =
                e.name === "ChunkedExecTimeoutError" ||
                (e.message &&
                  (e.message.includes("120000ms") ||
                    e.message.includes("60000ms") ||
              ...
              if (isCriticalError) {
                ...
                if (e.message === "GPUContextCreationError")
                  errorType = "GPU Context Creation Failed";
                else if (e.message === "HarnessError")
                  errorType = "Harness Error (Restarting)";
                else if (e.message.includes("WhiteScreen"))
                  errorType = "White Screen (browser crash)";
                else if (e.message.includes("GPUProcessCrash"))
                  errorType = "GPU Process Crash";
                const crashLabel = "[CRASH]";

                console.log(
                  `[Fail] ${errorType} for ${testFile} (${e.message}). Force-killing browser and restarting...`,
                );
                ...
                if (!isRestarting) {
                  ...
                  try {
                      ...
                      new Promise((_, reject) =>
                        setTimeout(
                          () =>
                            reject(
                              new Error(
                                "restartBrowserAndContext timeout 45000ms exceeded (after critical error)",
                              ),
                            ),
                          45000,
                        ),
                    ...
                    console.error(
                      `[Fail] Fatal error restarting browser: ${restartError.message}. Force-killing stray processes...`,
                    );
                    try {
                      this.forceKillBrowserProcessTree(
                        "post-critical restart hang",
                      );
                    } catch (e) {
                      console.log(
                        `[Debug] post-critical restart hang forceKill failed: ${e && e.message ? e.message : e}`,
                      );
                    }
                  } finally {
                    isRestarting = false;
                ...
                const crashLabel = "[CRASH]";
                console.log(
                  `[Fail] ${crashLabel} Timeout for ${testFile} (${e.message}). Force-killing browser and restarting...`,
                );
                ...
                if (!isRestarting) {
                  ...
                  try {
                      ...
                      new Promise((_, reject) =>
                        setTimeout(
                          () =>
                            reject(
                              new Error(
                                "restartBrowserAndContext timeout 45000ms exceeded (after WPT timeout)",
                              ),
                            ),
                          45000,
                        ),
                    ...
                    console.error(
                      `[Fail] Fatal error restarting browser after timeout: ${restartError.message}. Force-killing stray processes...`,
                    );
                    try {
                      this.forceKillBrowserProcessTree(
                        "post-timeout restart hang",
                      );
                    } catch (e) {
                      console.log(
                        `[Debug] post-timeout-restart-hang(catch) forceKill failed: ${e && e.message ? e.message : e}`,
                      );
                    }
                  } finally {
                    isRestarting = false;
                    try {
                      ...
                      console.log(
                        `[Debug] post-timeout-restart-hang(finally) forceKill failed: ${e && e.message ? e.message : e}`,
                      );
                    }
                  }
                }
              ...
              if (page && !page.isClosed()) {
                try {
                  ...
                  console.log(
                    `[Debug] chunkedExec finally page.close failed: ${e && e.message ? e.message : e}`,
                  );
                }
              }
            }
    ...
    if (failures.length > 0) {
      if (process.env.SKIP_RETRY === "true") {
        ...
        console.log(
          `[Info] Closing main browser context to ensure fresh environments for retries.`,
        );
        ...
        for (let i = 0; i < failures.length; i++) {
          ...
          while (attempt <= maxRetries) {
            ...
            try {
              ...
              const res = await this.runSingleWptTest(
                retryPage,
                testFile,
                -1,
                -1,
              ...
              try {
                const retryGpuLog = await this.getGpuCrashLogInfo(
                  retryInstance.context,
                );
                if (retryGpuLog.crashCount > 0) {
                  const crashLabel = "[CRASH]";
                  const crashDetails =
                    Array.isArray(retryGpuLog.crashDetails) &&
                    retryGpuLog.crashDetails.length > 0
                ...
                console.log(
                  `[Debug] retry GPU crash check failed (best effort): ${e && e.message ? e.message : e}`,
                );
              }
              ...
              if (res.result === "PASS") {
                ...
                if (this.compareTestResults(result, res)) {
                  console.log(
                    `[Warning]  Retry ${attempt} result matches previous failure. Stopping retries for this case.`,
                  );
                  result.retryHistory = retryHistory;
                  break;
                }
              ...
              if (isCriticalError) {
                const crashLabel = "[CRASH]";
                console.error(
                  `[Fail] ${crashLabel} Error during retry ${attempt}: ${e.message}`,
                );
                result.result = "CRASH";
                ...
                console.error(
                  `[Fail] Error during retry ${attempt}: ${e.message}`,
                );
                retryHistory.push({
                  attempt,
                  status: "ERROR",
              ...
              if (retryInstance) {
                try {
                  if (retryInstance.page && !retryInstance.page.isClosed())
                    await retryInstance.page.close();
                  if (retryInstance.context)
                    await retryInstance.context.close();
                  if (
                    retryInstance.browser &&
                    retryInstance.browser !== retryInstance.context
                  )
                    await retryInstance.browser.close();
                } catch (e) {
                  console.log(
                    `[Debug] retry-phase retryInstance close failed: ${e && e.message ? e.message : e}`,
                  );
                }
              }
            }
  ...
  async getGpuCrashLogInfo(context) {
    ...
    try {
      ...
      await Promise.race([
        gpuPage.goto("chrome://gpu", {
          waitUntil: "domcontentloaded",
          timeout: 10000,
        }),
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("chrome://gpu goto timeout")),
            12000,
          ),
        ),
      ]);
      ...
      const crashInfo = await Promise.race([
        gpuPage.evaluate((pattern) => {
          ...
          const gpuLogMessages = Array.from(
            infoViewHost.shadowRoot.querySelectorAll(
          ...
          for (let i = 0; i < gpuLogMessages.length; i++) {
            ...
            const detail =
              preLines.length > 0 ? `${preLines.join(" | ")} -> ${line}` : line;
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("chrome://gpu evaluate timeout")),
            8000,
          ),
        ),
      ]);
      ...
      console.log(
        `[Info] Checked chrome://gpu logs. Crash count: ${crashLines.length} ${crashDetails}`,
      );
      return {
      ...
      if (gpuPage && !gpuPage.isClosed()) {
        try {
          ...
          console.log(
            `[Debug] getGpuCrashLogInfo finally gpuPage.close failed: ${e && e.message ? e.message : e}`,
          );
        }
      }
    }
  ...
  async runSingleWptTest(page, testFile, index, totalFiles, retryCount = 0) {
    ...
    const runTest = async () => {
          ...
          new Promise((_, reject) =>
            setTimeout(
              () =>
                reject(
                  new Error(
                    `Timeout: ${label} exceeded ${ms}ms (likely WhiteScreen crash)`,
                  ),
                ),
              ms,
            ),
      ...
      const emergencyCleanup = async (reason) => {
        try {
          this.forceKillBrowserProcessTree(reason);
        } catch (e) {
          console.log(
            `[Debug] emergencyCleanup(${reason}) forceKill failed: ${e && e.message ? e.message : e}`,
          );
        }
        if (page && !page.isClosed()) {
          try {
            ...
            console.log(
              `[Debug] emergencyCleanup(${reason}) page.close failed: ${e && e.message ? e.message : e}`,
            );
          }
        }
      };
      ...
      try {
        ...
        console.log(
          `[Fail] page.goto failed for ${testName}: ${gotoErr.message}. Force-killing browser...`,
        );
      ...
      try {
        ...
        console.log(
          `[Debug] whitescreen check threw — assuming whitescreen: ${e && e.message ? e.message : e}`,
        );
        isWhiteScreen = true;
      }
      ...
      try {
        if (!isGpuProcessAlive()) {
          ...
          throw new Error(
            "GPUProcessCrash: gpu-process child no longer running",
          );
        }
      } catch (gpuErr) {
        if (gpuErr.message && gpuErr.message.startsWith("GPUProcessCrash"))
          throw gpuErr;
      ...
      let crashError = null;
      try {
        crashError = await withTimeout(
          page.evaluate(() => {
            const pre = document.evaluate(
              '//*[@id="summary"]/section/pre[1]',
              document,
              null,
            ...
            if (
              ...
              pre.textContent.includes(
                "Error: Unable to create context for gpu variant",
              )
            ) {
              return "GPUContextCreationError";
            }
            const summarySpan = document.evaluate(
              '//*[@id="summary"]/section/p/span',
              document,
              null,
              XPathResult.FIRST_ORDERED_NODE_TYPE,
              null,
        ...
        console.log(
          `[Debug] crashError check hung/failed — treating as whitescreen: ${e && e.message ? e.message : e}`,
        );
      ...
      while (Date.now() - pollStart < 57000) {
        ...
        try {
          ...
          console.log(
            `[Debug] poll evaluate threw — assuming whitescreen: ${e && e.message ? e.message : e}`,
          );
          state = "whitescreen";
        }
          ...
          try {
            if (!isGpuProcessAlive()) {
              await emergencyCleanup(
                "gpu process crashed during test execution",
              );
              throw new Error(
                "GPUProcessCrash: gpu-process child died mid-test",
              );
            }
          } catch (gpuErr) {
            if (gpuErr.message && gpuErr.message.startsWith("GPUProcessCrash"))
      ...
      const resData = await withTimeout(
        page.evaluate((scrapeDetails) => {
          const body = document.body.textContent;
          let subcases = { total: 0, passed: 0, failed: 0 };

          const pattern1 = body.match(
            /Found\s+(\d+)\s+tests?\s*(\d+)\s+Pass\s*(\d+)\s+Fail/i,
          );
          const pattern1AllPass = body.match(
            /Found\s+(\d+)\s+tests?\s*(\d+)\s+Pass(?!\s*\d+\s+Fail)/i,
          );
          ...
          if (pattern1) {
            ...
            if (patternSimpleFail)
              subcases.failed = parseInt(patternSimpleFail[1]);
            if (patternSimplePass)
              subcases.passed = parseInt(patternSimplePass[1]);
            subcases.total = subcases.passed + subcases.failed;
          } else {
          ...
          if (subcases.total === 0) {
            ...
            if (body.toLowerCase().includes("test") && allNumbers.length >= 2) {
              ...
              if (body.toLowerCase().includes("pass"))
          ...
          if (subcases.total === 0) {
            ...
            if (
              lowerBody.includes("complete") ||
              lowerBody.includes("finished")
            ) {
              subcases.total = 1;
              if (lowerBody.includes("fail") || lowerBody.includes("error"))
                subcases.failed = 1;
          ...
          else if (subcases.total > 0 && subcases.passed === subcases.total)
            resultStatus = "PASS";
          else if (body.includes("PASS")) {
            subcases.total = 1;
            subcases.passed = 1;
            resultStatus = "PASS";
          } else if (body.includes("FAIL")) {
            subcases.total = 1;
            subcases.failed = 1;
            resultStatus = "FAIL";
          }
          ...
          if (scrapeDetails && subcases.failed > 0) {
            ...
            if (resultsTable) {
              ...
              const allRows = tbody
                ...
                : resultsTable.querySelectorAll(":scope > tr");
      ...
      if (failedSubtests && failedSubtests.length > 0) {
        console.log(
          `[${testName}] Captured ${failedSubtests.length} failed subtest(s) details`,
        );
      }

      console.log(
        `[${backend}] [${testName}] ${resData.result}: ${resData.subcases.passed} PASS, ${resData.subcases.failed} FAIL`,
      );
      ...
      return {
        ...
        failedSubtests:
          failedSubtests && failedSubtests.length > 0
            ? failedSubtests
            : undefined,
        executionTime: "0.00",
      };
    ...
    try {
      ...
      return await Promise.race([
        ...
        new Promise((_, reject) =>
          setTimeout(
            () => reject(new Error("Timeout 120000ms exceeded")),
            120000,
          ),
        ),
      ]);
